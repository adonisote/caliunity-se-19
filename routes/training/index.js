...
trainingRouter.get('/training/:trainingId', (req, res) => {
  ...
  const training = req.user.userPastTrainings.find(training => training.trainingId === req.params.trainingId)
  console.log("First", training)
  if (!training) {
    return res.status(404).send(`Training id "${req.params.trainingId}" not found.`)
  }
